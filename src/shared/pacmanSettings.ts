...
export const PACMAN_MOVE_COOLDOWN_SEC = 0.15;
export const PACMAN_GHOST_MOVE_COOLDOWN_SEC = 0.20;
export const PACMAN_GHOST_SPAWN_STAGGER_SEC = 0.10;
...
export const PACMAN_DIR_DELTA: readonly { dc: number; dr: number }[] = [
  { dc: 0, dr: -1 },
  { dc: 0, dr: 1 },
  { dc: -1, dr: 0 },
  { dc: 1, dr: 0 },
...
export type PacmanMazeBuild = {
  walls: boolean[][];
  pellets: boolean[][];
  powerPellets: { col: number; row: number }[];
  ghostHouse: { col: number; row: number };
  playerSpawn: { col: number; row: number };
  ghostSpawns: { col: number; row: number }[];
  scatterTargets: { col: number; row: number }[];
  totalPellets: number;
};

function assertMazeDimensions(): void {
...
export function buildPacmanMaze(): PacmanMazeBuild {
  const walls: boolean[][] = [];
  const pellets: boolean[][] = [];
  const powerPellets: { col: number; row: number }[] = [];
  let ghostHouse = { col: 15, row: 5 };
  let playerSpawn = { col: 15, row: 15 };
  const ghostSpawns: { col: number; row: number }[] = [];
  let totalPellets = 0;

  for (let row = 0; row < PACMAN_ROWS; row++) {
    const wallLine: boolean[] = [];
    const pelletLine: boolean[] = [];
...
export function pacmanInBounds(col: number, row: number): boolean {
  return col >= 0 && row >= 0 && col < PACMAN_COLS && row < PACMAN_ROWS;
}

export function pacmanOppositeDir(d: PacmanDir): PacmanDir {
  if (d === 0) return 1;
  if (d === 1) return 0;
  if (d === 2) return 3;
  return 2;
...
export function pacmanCellCenterPx(col: number, row: number): { x: number; y: number } {
  return {
...
export function pickPacmanPlayerSpawns(
  playerCount: number,
