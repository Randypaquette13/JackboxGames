...
export type KartTrackGeometryFile = {
  outerWall: Vec2[];
  innerIslands: Vec2[][];
  bridgePolygon: Vec2[];
...
function softenClosed(points: Vec2[], passes: number): Vec2[] {
  let poly = points.slice();
...
export function buildProceduralKartTrackGeometry(): KartTrackGeometryFile {
  const outerWall = softenClosed(chaikinClosed(OUTER_WALL, 3), 1);
  const innerIslands = [softenClosed(chaikinClosed(INNER_ISLAND, 3), 1)];
  const bridgePolygon: Vec2[] = [];
  const underpassPolygon: Vec2[] = [];
