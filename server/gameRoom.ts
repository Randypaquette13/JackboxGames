...
import {
  clampTanksRoundsToWin,
  clampTanksTurnSec,
  resolveTanksRoundsToWin,
} from "../src/shared/tanksGameSettings.js";
import {
...
import {
  clampPacmanLivesPerPlayer,
} from "../src/shared/pacmanGameSettings.js";
import {
...
import {
  ...
  clearBombermanState,
  tickBombermanPlay,
  type BombermanBombSim,
  type BombermanFlameSim,
  type BombermanPlayerSim,
  type BombermanPowerUpSim,
} from "./bombermanRoom.js";
import type { BombermanCellKind } from "../src/shared/bombermanSettings.js";
import {
  bootstrapPacmanFromMenu,
  buildPacmanHostJson,
  clearPacmanState,
  tickPacmanPlay,
  type PacmanGhostSim,
  type PacmanPlayerSim,
} from "./pacmanRoom.js";
...
export { handleTanksPauseEdge } from "./tanksRoom.js";
...
export type Room = {
  ...
  /** Player id who may operate shared menus (minigame select, settings, results). */
  ...
  tanksTeamPick: Map<number, import("../src/shared/messages.js").TanksTeam>;
  tanksTeamAssignment: Map<number, import("../src/shared/messages.js").TanksTeam>;
  tanksPlayers: Map<number, TanksPlayer>;
  tanksTurnOrder: number[];
  tanksTurnIndex: number;
  tanksTurnPlayerId: number | null;
  tanksSubPhase: "aim" | "flight" | "explosion";
  tanksProjectile: import("./tanksRoom.js").TanksProjectile | null;
  tanksExplosion: import("./tanksRoom.js").TanksExplosion | null;
  tanksTurnSecLeft: number;
  tanksRedScore: number;
  tanksBlueScore: number;
  tanksSummaryEndTick: number | null;
  tanksKickoffCountdown: number | null;
  tanksPausedByPlayerId: number | null;
  tanksWinner: import("../src/shared/messages.js").TanksTeam | "tie" | null;
  bombermanCountdown: number | null;
  bombermanCells: BombermanCellKind[][];
  bombermanPlayers: Map<number, BombermanPlayerSim>;
  bombermanBombs: BombermanBombSim[];
  bombermanFlames: BombermanFlameSim[];
  bombermanPowerUps: BombermanPowerUpSim[];
  bombermanWinnerId: number | null;
  bombermanPausedByPlayerId: number | null;
  bombermanBanners: RaceWalkBannerJson[];
  bombermanDeathNotices: Map<number, { text: string; untilTick: number }>;
  pacmanCountdown: number | null;
  pacmanWalls: boolean[][];
  pacmanPellets: boolean[][];
  pacmanPowerPelletKeys: Set<string>;
  pacmanPelletsRemaining: number;
  pacmanTotalPellets: number;
  pacmanFrightenedSecLeft: number;
  pacmanChaseScatterTimer: number;
  pacmanChaseMode: boolean;
  pacmanGhostHouse: { col: number; row: number };
  pacmanPlayerSpawn: { col: number; row: number };
  pacmanPlayers: Map<number, PacmanPlayerSim>;
  pacmanGhosts: PacmanGhostSim[];
  pacmanPausedByPlayerId: number | null;
  pacmanBanners: RaceWalkBannerJson[];
  pacmanDeathNotices: Map<number, { text: string; untilTick: number }>;
  pacmanTeamCleared: boolean;
  pacmanTeamWiped: boolean;
};
...
export function createRoom(host: WebSocket, platforms: Platform[]): Room {
  return {
    ...
    tanksTeamPick: new Map(),
    tanksTeamAssignment: new Map(),
    tanksPlayers: new Map(),
    tanksTurnOrder: [],
    tanksTurnIndex: 0,
    tanksTurnPlayerId: null,
    tanksSubPhase: "aim",
    ...
    tanksRedScore: 0,
    tanksBlueScore: 0,
    tanksSummaryEndTick: null,
    tanksKickoffCountdown: null,
    tanksPausedByPlayerId: null,
    tanksWinner: null,
...
export function buildControllerState(room: Room, playerId: number): ControllerStateJson {
  ...
  const canTanksStart = nPlayers >= 2;
  const tkTeamMapForUi =
    room.phase === "tanks_team_select" ? room.tanksTeamPick : room.tanksTeamAssignment;
  const tkPlayer = room.tanksPlayers.get(playerId);
  const tanksHud =
    ...
    room.phase === "tanks_results"
      ? {
          teamSelect: room.phase === "tanks_team_select",
          myTeam: (tkTeamMapForUi.get(playerId) ?? room.tanksTeamPick.get(playerId)) ?? null,
          redIds: [...tkTeamMapForUi.entries()]
            .filter(([, t]) => t === "red")
            .map(([id]) => id)
            .sort((a, b) => a - b),
          blueIds: [...tkTeamMapForUi.entries()]
            .filter(([, t]) => t === "blue")
            .map(([id]) => id)
            .sort((a, b) => a - b),
          canStart: canTanksStart && room.phase === "tanks_team_select",
          isStarter: room.phase === "tanks_team_select" && canTanksStart,
          redScore: room.tanksRedScore,
          blueScore: room.tanksBlueScore,
          roundsToWin: resolveTanksRoundsToWin(room.gameSettings),
          isMyTurn: room.tanksTurnPlayerId === playerId && room.tanksSubPhase === "aim",
          isOut: tkPlayer ? !tkPlayer.alive : false,
          myAngle: tkPlayer?.angle ?? 0,
          myPower: tkPlayer?.power ?? 0.62,
  ...
  const bombermanHud =
    ...
    room.phase === "bomberman_results"
      ? {
          ...
          deathNotice:
            bmNotice && room.tick < bmNotice.untilTick
  ...
  const pacmanHud =
    room.phase === "pacman" || room.phase === "pacman_paused" || room.phase === "pacman_results"
      ? {
          ...
          frightenedSecLeft:
            room.pacmanFrightenedSecLeft > 0 ? room.pacmanFrightenedSecLeft : null,
          deathNotice:
            pmNotice && room.tick < pmNotice.untilTick
...
export function startPacmanFromMenu(room: Room): void {
  clearRaceWalkState(room);
  clearFroggerState(room);
  clearFootballState(room);
  clearAirHockeyState(room);
...
export function startBombermanFromMenu(room: Room): void {
  clearRaceWalkState(room);
  clearFroggerState(room);
  clearFootballState(room);
  clearAirHockeyState(room);
...
export function startTanksFromMenu(room: Room): void {
  room.kartCars.clear();
  room.kartWinnerId = null;
  room.kartCountdown = null;
  room.kartPaused = false;
...
export function tickSimulation(room: Room, dt: number): void {
  ...
  if (room.phase === "tanks_team_select" || room.phase === "tanks_results") {
    return;
  }
  if (room.phase === "tanks_summary") {
    tickTanksSummary(room);
    return;
  }
  if (room.phase === "tanks_paused") {
    return;
  }
  if (room.phase === "tanks") {
    tickTanksPlay(room, dt);
    return;
  }
...
function isResultsPhase(phase: Room["phase"]): boolean {
  return (
    phase === "kart_results" ||
...
function isSharedMenuActive(room: Room): boolean {
  return room.phase === "menu" || isResultsPhase(room.phase);
}

function clearMenuController(room: Room): void {
  room.menuControllerId = null;
}

export function onMenuControllerPlayerRemoved(room: Room, playerId: number): void {
  if (room.menuControllerId === playerId) clearMenuController(room);
}

function menuControllerName(room: Room): string | null {
  if (room.menuControllerId === null) return null;
  const p = room.players.get(room.menuControllerId);
  return p?.name ?? `P${room.menuControllerId}`;
}

function authorizeMenuIntent(room: Room, playerId: number, intent: ClientIntent): boolean {
  if (!isSharedMenuActive(room)) return true;
...
export function applyIntent(room: Room, _playerId: number, intent: ClientIntent): void {
  if (
    ...
    intent.type === "tanks_results" ||
    intent.type === "bomberman_results" ||
    intent.type === "pacman_results"
  ) {
    if (!authorizeMenuIntent(room, _playerId, intent)) return;
  }
  ...
  switch (intent.type) {
    ...
    case "menu_confirm": {
      ...
      if (room.phase === "tanks_results") {
        const actions = ["play_again", "minigame_menu", "add_controllers"] as const;
        const action = actions[room.menuIndex % 3];
        if (action === "play_again") {
          clearMenuController(room);
          startTanksFromMenu(room);
        } else if (action === "minigame_menu") {
          goToMinigameMenu(room);
          room.stubId = null;
          clearTanksState(room);
      ...
      if (room.phase === "menu") {
        ...
        else if (id === "tanks") startTanksFromMenu(room);
    ...
    case "dodgeball_start":
      ...
      break;
    case "tanks_pick_team":
      if (room.phase === "tanks_team_select") {
        room.tanksTeamPick.set(_playerId, intent.team);
      }
      break;
    case "tanks_start":
      if (room.phase === "tanks_team_select") {
    ...
    case "game_settings_patch": {
      ...
      if (Object.prototype.hasOwnProperty.call(p, "tanksRoundsToWin")) {
        const v = p.tanksRoundsToWin;
        if (typeof v === "number" && Number.isFinite(v)) {
          room.gameSettings.tanksRoundsToWin = clampTanksRoundsToWin(v);
        }
      }
      if (Object.prototype.hasOwnProperty.call(p, "tanksTurnSec")) {
        const v = p.tanksTurnSec;
        if (typeof v === "number" && Number.isFinite(v)) {
          room.gameSettings.tanksTurnSec = clampTanksTurnSec(v);
        }
      }
    ...
    case "dodgeball_results":
      ...
      break;
    case "tanks_results":
      if (room.phase !== "tanks_results") break;
      if (intent.action === "play_again") {
        clearMenuController(room);
        startTanksFromMenu(room);
      } else if (intent.action === "minigame_menu") {
    ...
    case "pause_resume":
      ...
      } else if (room.phase === "tanks_paused") {
        room.phase = "tanks";
        room.tanksPausedByPlayerId = null;
        for (const p of room.tanksPlayers.values()) {
          p.prevPauseHeld = false;
    ...
    case "pause_to_menu":
      ...
      break;
    case "pacman_results":
      if (room.phase !== "pacman_results") break;
      if (intent.action === "play_again") {
        clearMenuController(room);
        startPacmanFromMenu(room);
      } else if (intent.action === "minigame_menu") {
