...
import type { DodgeballTeam } from "../src/shared/messages.js";
...
import {
  DODGEBALL_BALL_COUNT,
  DODGEBALL_BALL_DRAG,
  DODGEBALL_BALL_R,
  DODGEBALL_CATCH_SEC,
  DODGEBALL_CENTER_BALL_JITTER,
  DODGEBALL_COURT_X0,
  DODGEBALL_COURT_X1,
  DODGEBALL_COURT_Y0,
  DODGEBALL_COURT_Y1,
  DODGEBALL_KICKOFF_COUNTDOWN_SEC,
  DODGEBALL_LIVE_THROW_SEC,
  DODGEBALL_MID_X,
  DODGEBALL_PLAYER_BOUNCE_ITERS,
  DODGEBALL_PLAYER_BOUNCE_STRENGTH,
  DODGEBALL_PLAYER_R,
  DODGEBALL_WALL_RESTITUTION,
} from "../src/shared/dodgeballSettings.js";
import { Btn } from "../src/shared/protocol.js";
import type { Room } from "./gameRoom.js";

export type DodgeballPlayer = {
  x: number;
  y: number;
  team: DodgeballTeam;
  alive: boolean;
  ...
  /** While > tick: rooted in catch stance (movement disabled). */
  catchUntilTick: number;
};

export type DodgeballBall = {
  id: number;
  x: number;
  y: number;
  vx: number;
  ...
  /** Throwing team while live; null when neutral. */
  liveTeam: DodgeballTeam | null;
  thrownByPlayerId: number | null;
  liveUntilTick: number;
};
...
const CENTER_BALL_OFFSETS: { dx: number; dy: number }[] = [
  { dx: 0, dy: 0 },
  { dx: -22, dy: -14 },
  { dx: 22, dy: -14 },
  { dx: -22, dy: 14 },
...
export function dodgeballPhases(room: Room): boolean {
  const ph = room.phase;
  return (
...
export function clearDodgeballState(room: Room): void {
...
export function bootstrapDodgeballFromMenu(room: Room): void {
...
function playerStartY(index: number, total: number): number {
...
export function dodgeballTryStart(room: Room): boolean {
  if (room.phase !== "dodgeball_team_select") return false;
  const ids = [...room.players.keys()].sort((a, b) => a - b);
  if (ids.length < 2) return false;
  room.dodgeballTeamAssignment.clear();
  for (const id of ids) {
    const p = room.dodgeballTeamPick.get(id);
    if (p === "red" || p === "blue") room.dodgeballTeamAssignment.set(id, p);
  }
  let reds = [...room.dodgeballTeamAssignment.values()].filter((t) => t === "red").length;
  let blues = ids.length - reds;
  for (const id of ids) {
    if (room.dodgeballTeamAssignment.has(id)) continue;
    const choose: DodgeballTeam = reds <= blues ? "red" : "blue";
...
function spawnCenterBalls(room: Room): void {
  room.dodgeballBalls = [];
...
function initRoundLayout(room: Room, reviveAll: boolean): void {
  if (reviveAll) {
    room.dodgeballRedElimOrder = [];
    room.dodgeballBlueElimOrder = [];
  }
...
export function tickDodgeballSummary(room: Room): void {
  if (room.phase !== "dodgeball_summary" || room.dodgeballSummaryEndTick === null) return;
  if (room.tick < room.dodgeballSummaryEndTick) return;
...
export function handleDodgeballPauseEdge(
  room: Room,
  playerId: number,
  p: DodgeballPlayer,
  pauseHeld: boolean
...
function clampPlayer(p: DodgeballPlayer): void {
  const pr = DODGEBALL_PLAYER_R;
  if (p.team === "red") {
...
function resolvePlayerCrowding(room: Room): void {
  ...
  for (const id of ids) {
    ...
    if (p?.alive) clampPlayer(p);
  }
}

function ballIsLive(ball: DodgeballBall, tick: number): boolean {
  return ball.liveTeam !== null && tick < ball.liveUntilTick && ball.carrierId === null;
}

function elimQueueForTeam(room: Room, team: DodgeballTeam): number[] {
  return team === "red" ? room.dodgeballRedElimOrder : room.dodgeballBlueElimOrder;
}

function eliminatePlayer(room: Room, pid: number, team: DodgeballTeam): void {
...
function reviveFirstEliminatedTeammate(room: Room, team: DodgeballTeam): void {
  const q = elimQueueForTeam(room, team);
  while (q.length > 0) {
    const pid = q.shift()!;
    const p = room.dodgeballPlayers.get(pid);
...
function aliveCount(room: Room, team: DodgeballTeam): number {
  let n = 0;
...
function endMatch(room: Room, winner: DodgeballTeam | "tie"): void {
...
function endRound(room: Room, winner: DodgeballTeam): void {
  if (winner === "red") room.dodgeballRedScore++;
  else room.dodgeballBlueScore++;

  const need = resolveDodgeballRoundsToWin(room.gameSettings);
  if (
    (winner === "red" && room.dodgeballRedScore >= need) ||
...
function checkRoundOver(room: Room): boolean {
...
function playerHoldingBall(room: Room, pid: number): DodgeballBall | null {
  for (const ball of room.dodgeballBalls) {
    if (ball.carrierId === pid) return ball;
  }
  return null;
}

function assignBallToPlayer(ball: DodgeballBall, pid: number, p: DodgeballPlayer): void {
  ball.carrierId = pid;
  ball.x = p.x;
  ball.y = p.y;
  ball.vx = 0;
...
function tryPickupBall(
  ...
  if (!p.alive || ball.carrierId !== null) return false;
  if (playerHoldingBall(room, pid)) return false;
  const d = Math.hypot(p.x - ball.x, p.y - ball.y);
  if (d > PICKUP_DIST) return false;
  assignBallToPlayer(ball, pid, p);
  return true;
}

function onSuccessfulCatch(
  room: Room,
  catcherId: number,
  catcher: DodgeballPlayer,
  ball: DodgeballBall
): void {
  const throwerId = ball.thrownByPlayerId;
  const throwerTeam = ball.liveTeam;
  const opponentLiveThrow =
    throwerTeam !== null &&
...
function resolveBallPlayerContacts(room: Room): void {
...
function clampLooseBall(ball: DodgeballBall): void {
...
function resolveBallBallSeparation(room: Room): void {
...
function nearestOpponentUnitDir(
  room: Room,
  team: DodgeballTeam,
  x: number,
  y: number
  ...
  for (const opp of room.dodgeballPlayers.values()) {
    if (!opp.alive || opp.team === team) continue;
    const dx = opp.x - x;
    const dy = opp.y - y;
    const d = Math.hypot(dx, dy);
    if (d < 1e-8) continue;
  ...
  if (!Number.isFinite(bestDist) || bestDist === Infinity) return null;
  return { x: tx, y: ty };
}

function blendUnitDir(
  ax: number,
  ay: number,
  bx: number,
  by: number,
  ...
  if (len < 1e-8) return { x: bx, y: by };
  return { x: x / len, y: y / len };
}

/** Gently curve live throws toward the closest living opponent. */
function applyLiveThrowHoming(room: Room, ball: DodgeballBall, dt: number): void {
  if (!ballIsLive(ball, room.tick) || ball.liveTeam === null) return;
...
function maybeTryThrow(
  room: Room,
  pid: number,
  p: DodgeballPlayer,
  ball: DodgeballBall,
  pl: { input: { buttons: number; footballVx?: number; footballVy?: number } }
): void {
  const passHeld = (pl.input.buttons & Btn.Pass) !== 0;
  const edge = passHeld && !p.prevPassHeld;
  p.prevPassHeld = passHeld;
  if (!edge || !p.alive) return;
...
function maybeTryCatch(
  room: Room,
  pid: number,
  p: DodgeballPlayer,
  pl: { input: { buttons: number } }
): void {
  const passHeld = (pl.input.buttons & Btn.Pass) !== 0;
  const edge = passHeld && !p.prevPassHeld;
  p.prevPassHeld = passHeld;
  if (!edge || !p.alive) return;
  if (playerHoldingBall(room, pid)) return;
  p.catchUntilTick = room.tick + Math.floor(TICK_RATE * DODGEBALL_CATCH_SEC);
}

export function tickDodgeballPlay(room: Room, dt: number): void {
  if (room.phase !== "dodgeball") return;

  if (room.dodgeballKickoffCountdown !== null && room.dodgeballKickoffCountdown > 0) {
    room.dodgeballKickoffCountdown -= dt;
    if (room.dodgeballKickoffCountdown <= 0) room.dodgeballKickoffCountdown = null;
    return;
  }
...
export function buildDodgeballHostJson(
  room: Room
): import("../src/shared/messages.js").HostStateJson["dodgeball"] {
  if (
    room.phase !== "dodgeball_team_select" &&
    room.phase !== "dodgeball_summary" &&
    room.phase !== "dodgeball" &&
  ...
  if (
    ...
    for (const [pid, p] of room.dodgeballPlayers) {
      const pl = room.players.get(pid);
      let holdingBallId: number | null = null;
      for (const ball of room.dodgeballBalls) {
        if (ball.carrierId === pid) {
  ...
  return {
    ...
    kickoffCountdown:
      room.dodgeballKickoffCountdown !== null && room.dodgeballKickoffCountdown > 0
