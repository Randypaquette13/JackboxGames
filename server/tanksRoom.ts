...
import type { TanksTeam } from "../src/shared/messages.js";
...
import {
  ...
  TANKS_GRAVITY,
  TANKS_HIT_RADIUS,
  TANKS_KICKOFF_COUNTDOWN_SEC,
  TANKS_MAX_DEPRESSION,
  TANKS_MAX_ELEVATION,
  TANKS_MAX_POWER,
  TANKS_MAX_VELOCITY,
  TANKS_MID_X,
  ...
  TANKS_ANGLE_RATE,
  TANKS_POWER_RATE,
  TANKS_RED_DEFAULT_ANGLE,
} from "../src/shared/tanksSettings.js";
import { Btn } from "../src/shared/protocol.js";
import type { Room } from "./gameRoom.js";

export type TanksPlayer = {
  x: number;
  y: number;
  team: TanksTeam;
  alive: boolean;
  ...
  prevAimDown: boolean;
  prevPowerUp: boolean;
  prevPowerDown: boolean;
};

export type TanksProjectile = {
  x: number;
  y: number;
  vx: number;
  vy: number;
...
export function tanksPhases(room: Room): boolean {
  const ph = room.phase;
  return (
    ph === "tanks_team_select" ||
    ph === "tanks_summary" ||
    ph === "tanks" ||
    ph === "tanks_paused" ||
    ph === "tanks_results"
...
export function clearTanksState(room: Room): void {
...
export function bootstrapTanksFromMenu(room: Room): void {
...
function tankStartY(index: number, total: number): number {
...
function clampAngle(team: TanksTeam, angle: number): number {
  if (team === "red") {
...
function defaultAngle(team: TanksTeam): number {
  return team === "red" ? TANKS_RED_DEFAULT_ANGLE : TANKS_BLUE_DEFAULT_ANGLE;
}

function defaultPower(): number {
  return 0.62;
}

export function tanksTryStart(room: Room): boolean {
  if (room.phase !== "tanks_team_select") return false;
  const ids = [...room.players.keys()].sort((a, b) => a - b);
  if (ids.length < 2) return false;
  room.tanksTeamAssignment.clear();
  for (const id of ids) {
    const p = room.tanksTeamPick.get(id);
    if (p === "red" || p === "blue") room.tanksTeamAssignment.set(id, p);
  }
  let reds = [...room.tanksTeamAssignment.values()].filter((t) => t === "red").length;
  let blues = ids.length - reds;
  for (const id of ids) {
    if (room.tanksTeamAssignment.has(id)) continue;
    const choose: TanksTeam = reds <= blues ? "red" : "blue";
...
function buildTurnOrder(room: Room): void {
  ...
  for (let i = 0; i < maxLen; i++) {
    if (i < redIds.length) order.push(redIds[i]!);
    if (i < blueIds.length) order.push(blueIds[i]!);
  }
...
function aliveCount(room: Room, team: TanksTeam): number {
  let n = 0;
...
function endMatch(room: Room, winner: TanksTeam | "tie"): void {
...
function endRound(room: Room, winner: TanksTeam): void {
  if (winner === "red") room.tanksRedScore++;
  else room.tanksBlueScore++;

  const need = resolveTanksRoundsToWin(room.gameSettings);
  if (
    (winner === "red" && room.tanksRedScore >= need) ||
...
export function tickTanksSummary(room: Room): void {
  if (room.phase !== "tanks_summary" || room.tanksSummaryEndTick === null) return;
  if (room.tick < room.tanksSummaryEndTick) return;
  room.phase = "tanks";
  room.tanksKickoffCountdown = TANKS_KICKOFF_COUNTDOWN_SEC;
  room.tanksSummaryEndTick = null;
  initRoundLayout(room, true);
...
export function handleTanksPauseEdge(
  room: Room,
  playerId: number,
  p: TanksPlayer,
  pauseHeld: boolean
...
function muzzlePoint(p: TanksPlayer): { x: number; y: number } {
...
function fireProjectile(room: Room, pid: number, p: TanksPlayer): void {
...
function applyExplosion(room: Room, x: number, y: number): void {
  room.tanksExplosion = {
    x,
    y,
...
function finishExplosion(room: Room): void {
  room.tanksExplosion = null;
...
function processAimInput(
  ...
  if (room.tanksTurnPlayerId !== pid || room.tanksSubPhase !== "aim") return;
  ...
  if (aimUp) p.angle -= TANKS_ANGLE_RATE * dt;
  if (aimDown) p.angle += TANKS_ANGLE_RATE * dt;
  p.angle = clampAngle(p.team, p.angle);

  if (powerUp) p.power += TANKS_POWER_RATE * dt;
  if (powerDown) p.power -= TANKS_POWER_RATE * dt;
...
function tickProjectile(room: Room, dt: number): void {
...
export function tickTanksPlay(room: Room, dt: number): void {
  if (room.phase !== "tanks") return;

  if (room.tanksKickoffCountdown !== null && room.tanksKickoffCountdown > 0) {
    room.tanksKickoffCountdown -= dt;
    if (room.tanksKickoffCountdown <= 0) room.tanksKickoffCountdown = null;
    return;
  }
  ...
  if (room.tanksSubPhase === "aim" && room.tanksTurnPlayerId !== null) {
    ...
    if (room.tanksTurnSecLeft <= 0) {
      ...
      if (p?.alive) fireProjectile(room, room.tanksTurnPlayerId, p);
    }
  }
...
export function buildTanksHostJson(
  room: Room
): import("../src/shared/messages.js").HostStateJson["tanks"] {
  if (
    room.phase !== "tanks_team_select" &&
    room.phase !== "tanks_summary" &&
    room.phase !== "tanks" &&
  ...
  return {
    ...
    kickoffCountdown:
      room.tanksKickoffCountdown !== null && room.tanksKickoffCountdown > 0
