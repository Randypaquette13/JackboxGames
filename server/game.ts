...
import { type Platform, PLATFORMS } from "../src/shared/level.js";
import type { PlayerSnapshot } from "../src/shared/protocol.js";

export type { Platform } from "../src/shared/level.js";
...
export type SimPlayer = {
  ...
  /** Latest controller input (updated when INPUT arrives). */
  input: PlayerInput;
};
...
export function stepPlayer(p: SimPlayer, platforms: Platform[]): void {
  ...
  for (const pl of platforms) {
    ...
    if (p.vx > 0.01) p.x = pl.x - PLAYER_W;
    else if (p.vx < -0.01) p.x = pl.x + pl.w;
    else if (p.x + PLAYER_W * 0.5 < pl.x + pl.w * 0.5) p.x = pl.x - PLAYER_W;
    else p.x = pl.x + pl.w;
    p.vx = 0;
  }
