...
import { resolveFootballMaxPlayerSpeed, resolveFootballPeriodSec, resolveFootballTdToWin, } from "../src/shared/footballGameSettings.js";
import { FOOTBALL_BALL_R, FOOTBALL_BLUE_EZ_X0, FOOTBALL_FIELD_MARGIN_Y, FOOTBALL_FIELD_X0, FOOTBALL_FIELD_X1, FOOTBALL_KICKOFF_X_INSET, FOOTBALL_KICKOFF_BALL_JITTER, FOOTBALL_KICKOFF_COUNTDOWN_SEC, FOOTBALL_LIVE_BALL_X0, FOOTBALL_LIVE_BALL_X1, FOOTBALL_POST_TD_BALL_FRAC_FROM_MID, FOOTBALL_OPPONENT_TACKLE_PICKUP_LOCK_SEC, FOOTBALL_PASS_SPEED, FOOTBALL_POST_PASS_GLOBAL_PICKUP_LOCK_SEC, FOOTBALL_PLAYER_BOUNCE_CONTACT_SPEED_MUL, FOOTBALL_PLAYER_BOUNCE_RECOVERY_PER_SEC, FOOTBALL_PASS_TEAMMATE_LOCK_SEC, FOOTBALL_PLAYER_R, FOOTBALL_RED_EZ_X1, FOOTBALL_TACKLE_PICKUP_LOCK_SEC, } from "../src/shared/footballSettings.js";
...
function resolveNonCarrierPlayerBounces(room, carrierId) {
    ...
    for (let iter = 0; iter < PLAYER_BOUNCE_ITERS; iter++) {
        for (let i = 0; i < ids.length; i++) {
            const idA = ids[i];
            if (carrierId !== null && idA === carrierId)
                continue;
            ...
            for (let j = i + 1; j < ids.length; j++) {
                const idB = ids[j];
                if (carrierId !== null && idB === carrierId)
                    continue;
    ...
    for (const id of ids) {
        if (carrierId !== null && id === carrierId)
            continue;
...
export function footballPhases(room) {
    const ph = room.phase;
    return (ph === "football_team_select" ||
        ph === "football_summary" ||
        ph === "football" ||
        ph === "football_paused" ||
        ph === "football_results");
}
export function clearFootballState(room) {
...
export function bootstrapFootballFromMenu(room) {
...
function kickoffPlayerY(index, total) {
 ...
 */
function initKickoffLayout(room, receivingTeam) {
    ...
    if (receivingTeam === null) {
        ...
        const by = MID_Y + (Math.random() - 0.5) * 2 * yJitterMax;
        room.footballBall.y = Math.max(FIELD_Y0 + FOOTBALL_BALL_R, Math.min(FIELD_Y1 - FOOTBALL_BALL_R, by));
    ...
    const maxStaggerTowardMid = Math.min(FOOTBALL_RED_EZ_X1 - pr - kickoffRedX, kickoffBlueX - (FOOTBALL_BLUE_EZ_X0 + pr), kickoffRedX - pr, WORLD_W - pr - kickoffBlueX);
...
export function tickFootballSummary(room) {
    if (room.phase !== "football_summary" || room.footballSummaryEndTick === null)
        return;
    if (room.tick < room.footballSummaryEndTick)
        return;
    room.phase = "football";
...
export function handleFootballPauseEdge(room, playerId, fb, pauseHeld) {
...
function footballPickupBannedForTeam(room, team) {
    const t = room.tick;
    if (t < room.footballPickupFreezeUntilTick)
        return true;
    const mate = room.footballPickupLockTeam;
    if (mate !== null && team === mate && t < room.footballPickupLockUntilTick)
        return true;
    const oppTeam = room.footballOpponentPickupLockTeam;
    if (oppTeam !== null && team === oppTeam && t < room.footballOpponentPickupLockUntilTick)
        return true;
    return false;
}
function maybePickup(room, pid, a) {
    const b = room.footballBall;
    if (b.carrierId !== null || b.carrierId === pid)
        return;
    if (room.tick < room.footballPickupFreezeUntilTick)
        return;
    const lockTeam = room.footballPickupLockTeam;
    const lockUntil = room.footballPickupLockUntilTick;
    if (lockTeam !== null && a.team === lockTeam && room.tick < lockUntil)
        return;
    const oppLockTeam = room.footballOpponentPickupLockTeam;
    const oppLockUntil = room.footballOpponentPickupLockUntilTick;
    if (oppLockTeam !== null && a.team === oppLockTeam && room.tick < oppLockUntil)
        return;
...
function resolveFootballOvertime(room) {
    let w = "tie";
    if (room.footballRedScore > room.footballBlueScore)
        w = "red";
    else if (room.footballBlueScore > room.footballRedScore)
        w = "blue";
    endMatch(room, w);
}
...
function checkTouchdown(room, pid, carrierTeam) {
    const b = room.footballBall;
...
function doTackle(room, carrierTeam) {
    if (room.footballTimerExpired) {
        resolveFootballOvertime(room);
        return;
    }
    ...
    const defendingTeam = carrierTeam === "red" ? "blue" : "red";
...
function maybeTryPass(room, carrierId, carrierAth, carrierTeam) {
...
export function tickFootballPlay(room, dt) {
    ...
    if (carrierId !== null) {
        ...
        if (room.footballBall.carrierId !== carrierId)
            return;
        ...
        if (room.phase !== "football" || room.footballBall.carrierId !== carrierId)
            return;
        for (const [oid, oat] of room.footballAthletes) {
            if (oid === carrierId || oat.team === tm)
                continue;
        ...
        if (maybeTryPass(room, carrierId, carrierNow, tm))
            return;
        return;
    }
    b.x += b.vx * dt;
...
export function buildFootballHostJson(room) {
    if (room.phase !== "football_team_select" &&
        room.phase !== "football_summary" &&
        room.phase !== "football" &&
        room.phase !== "football_paused" &&
    ...
    const rosterRed = room.phase === "football_team_select"
        ? red
        : [...room.footballTeamAssignment.entries()]
            .filter(([, t]) => t === "red")
            .map(([id]) => rosterSlot(room, id))
            .sort((x, y) => x.playerId - y.playerId);
    const rosterBlue = room.phase === "football_team_select"
        ? blue
        : [...room.footballTeamAssignment.entries()]
            .filter(([, t]) => t === "blue")
            .map(([id]) => rosterSlot(room, id))
    ...
    return {
        ...
        kickoffCountdown: room.footballKickoffCountdown !== null && room.footballKickoffCountdown > 0
        ...
        winner: room.phase === "football_results" ? (room.footballWinner ?? "tie") : null,
        field: {
