...
export function clearPacmanState(room) {
...
export function bootstrapPacmanFromMenu(room) {
    room.phase = "pacman";
    room.stubId = null;
    room.showQr = false;
...
function isWall(room, col, row) {
    if (!pacmanInBounds(col, row)) {
        if (row === PACMAN_TUNNEL_ROW)
            return false;
        return true;
    }
...
function wrapCol(col, row) {
    if (row !== PACMAN_TUNNEL_ROW)
        return col;
    if (col < 0)
        return PACMAN_COLS - 1;
    if (col >= PACMAN_COLS)
        return 0;
    return col;
}
function canMove(room, col, row, dir) {
    const { dc, dr } = PACMAN_DIR_DELTA[dir];
    let nc = col + dc;
    const nr = row + dr;
    nc = wrapCol(nc, nr);
    if (nr < 0 || nr >= PACMAN_ROWS)
        return false;
    return !isWall(room, nc, nr);
}
function legalDirs(room, col, row, curDir) {
    const out = [];
    for (let d = 0; d < 4; d++) {
        const dir = d;
        if (!canMove(room, col, row, dir))
            continue;
        if (out.length > 0 && dir === pacmanOppositeDir(curDir))
            continue;
        out.push(dir);
    }
    if (out.length === 0) {
        const rev = pacmanOppositeDir(curDir);
        if (canMove(room, col, row, rev))
            out.push(rev);
    }
    return out;
}
...
function nearestAlivePlayer(room, from) {
    let best = null;
    let bestD = Infinity;
...
function applyMove(col, row, dir) {
    const { dc, dr } = PACMAN_DIR_DELTA[dir];
    let nc = col + dc;
    const nr = row + dr;
    nc = wrapCol(nc, nr);
...
function tryCollectPellet(room, p) {
...
function checkPacmanWin(room) {
    if (room.pacmanPelletsRemaining > 0)
        return;
...
function allPlayersOutOfLives(room) {
    if (room.pacmanPlayers.size === 0)
        return false;
    for (const p of room.pacmanPlayers.values()) {
        if (p.lives > 0)
            return false;
...
function checkPacmanTeamWipe(room) {
    if (room.phase !== "pacman")
        return;
    if (!allPlayersOutOfLives(room))
        return;
...
function dirFromInput(aimUp, aimDown, leftHeld, rightHeld) {
    if (aimUp)
        return 0;
    if (aimDown)
        return 1;
...
function stepPlayer(room, p) {
    if (!p.alive)
        return;
    let tryDir = p.queuedDir ?? p.dir;
    if (!canMove(room, p.col, p.row, tryDir))
        tryDir = p.dir;
    if (!canMove(room, p.col, p.row, tryDir))
        return;
...
function ghostTarget(room, g) {
    if (g.mode === "eaten")
        return { ...room.pacmanGhostHouse };
...
function stepGhost(room, g) {
    ...
    for (const [pid, p] of room.pacmanPlayers) {
        ...
        if (g.mode === "frightened") {
            ...
            if (scorer)
                scorer.score += PACMAN_GHOST_EAT_POINTS;
...
function tickGhostPlayerCollisions(room) {
...
export function handlePacmanPauseEdge(room, playerId, p, pauseHeld) {
...
export function buildPacmanHostJson(room) {
...
export function pacmanOnPlayerRemoved(room, playerId) {
    room.pacmanPlayers.delete(playerId);
    room.pacmanDeathNotices.delete(playerId);
    if (room.pacmanPausedByPlayerId === playerId)
        room.pacmanPausedByPlayerId = null;
}
