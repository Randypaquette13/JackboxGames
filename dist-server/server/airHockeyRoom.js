...
import { resolveAirHockeyGoalsToWin, resolveAirHockeyPeriodSec, } from "../src/shared/airHockeyGameSettings.js";
import { AIR_HOCKEY_FACEOFF_JITTER, AIR_HOCKEY_GOAL_MOUTH_FRAC, AIR_HOCKEY_KICKOFF_COUNTDOWN_SEC, AIR_HOCKEY_MALLET_RESTITUTION, AIR_HOCKEY_MALLET_VELOCITY_TRANSFER, AIR_HOCKEY_MALLET_VELOCITY_TRANSFER_MAX, AIR_HOCKEY_MALLET_VELOCITY_TRANSFER_STEP, AIR_HOCKEY_MID_X, AIR_HOCKEY_PLAYER_BOUNCE_ITERS, AIR_HOCKEY_PLAYER_BOUNCE_STRENGTH, AIR_HOCKEY_PLAYER_R, AIR_HOCKEY_PUCK_DRAG, AIR_HOCKEY_PUCK_MAX_SPEED, AIR_HOCKEY_PUCK_R, AIR_HOCKEY_RINK_X0, AIR_HOCKEY_RINK_X1, AIR_HOCKEY_RINK_Y0, AIR_HOCKEY_RINK_Y1, AIR_HOCKEY_WALL_RESTITUTION, } from "../src/shared/airHockeySettings.js";
const SUMMARY_SEC = 3;
...
export function airHockeyPhases(room) {
    const ph = room.phase;
    return (ph === "air_hockey_team_select" ||
        ph === "air_hockey_summary" ||
        ph === "air_hockey" ||
        ph === "air_hockey_paused" ||
        ph === "air_hockey_results");
}
export function clearAirHockeyState(room) {
...
export function bootstrapAirHockeyFromMenu(room) {
...
export function airHockeyTryStart(room) {
...
function malletStartY(index, total) {
...
function initFaceoffLayout(room) {
...
export function tickAirHockeySummary(room) {
    if (room.phase !== "air_hockey_summary" || room.airHockeySummaryEndTick === null)
        return;
    if (room.tick < room.airHockeySummaryEndTick)
        return;
...
export function handleAirHockeyPauseEdge(room, playerId, m, pauseHeld) {
...
function resolveMalletCrowding(room) {
...
function resolveVelocityTransfer(room) {
    return Math.min(AIR_HOCKEY_MALLET_VELOCITY_TRANSFER_MAX, AIR_HOCKEY_MALLET_VELOCITY_TRANSFER +
...
function resolvePuckMalletCollisions(room) {
    const p = room.airHockeyPuck;
...
function onGoal(room, scoringTeam) {
    const goalsNeed = resolveAirHockeyGoalsToWin(room.gameSettings);
    if (scoringTeam === "red")
        room.airHockeyRedScore++;
    else
        room.airHockeyBlueScore++;
...
export function buildAirHockeyHostJson(room) {
    if (room.phase !== "air_hockey_team_select" &&
        room.phase !== "air_hockey_summary" &&
        room.phase !== "air_hockey" &&
        room.phase !== "air_hockey_paused" &&
    ...
    const rosterRed = room.phase === "air_hockey_team_select"
        ? red
        : [...room.airHockeyTeamAssignment.entries()]
            .filter(([, t]) => t === "red")
            .map(([id]) => rosterSlot(room, id))
            .sort((x, y) => x.playerId - y.playerId);
    const rosterBlue = room.phase === "air_hockey_team_select"
        ? blue
        : [...room.airHockeyTeamAssignment.entries()]
            .filter(([, t]) => t === "blue")
            .map(([id]) => rosterSlot(room, id))
    ...
    return {
        ...
        kickoffCountdown: room.airHockeyKickoffCountdown !== null && room.airHockeyKickoffCountdown > 0
