...
export function froggerSectionWeights(bandsGenerated) {
...
export function pickFroggerSectionKind(bandsGenerated) {
...
export function froggerClampX(cx) {
    const half = FROGGER_FROG_SIZE / 2;
    return Math.max(half + 4, Math.min(WORLD_W - half - 4, cx));
}
export function froggerCarSpawnIntervalSec(bandsGenerated) {
