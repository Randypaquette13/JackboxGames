...
export const BOMMERMAN_CORNER_SPAWNS = [
    { col: 1, row: 1 },
...
export const BOMMERMAN_EXTRA_SPAWN_SLOTS = [
    { col: 11, row: 1 },
 ...
 */
export function bombermanSpawnPocketCells(col, row) {
    const cells = [];
    const add = (c, r) => {
        if (isInner(c, r))
...
export function pickBombermanSpawns(playerCount) {
...
export function bombermanMoveCooldownSec(speedTier) {
...
export function bombermanCellCenterPx(col, row) {
    return {
...
export function buildBombermanArena(playerCount) {
...
export function pickBombermanPowerKind() {
    const r = Math.random();
    if (r < 1 / 3)
        return "bomb";
    if (r < 2 / 3)
