...
export const FOOTBALL_MAG_TIERS = 4;
export const FOOTBALL_DIR_STEPS = 64;
/** Speed multipliers per mag tier: stop, walk, run, full. */
export const FOOTBALL_MAG_SPEED_FRAC = [0, 1 / 3, 2 / 3, 1];
/**
...
export const FOOTBALL_JOYSTICK_LEN_WALK_END = FOOTBALL_JOYSTICK_LEN_DEADZONE + FOOTBALL_JOYSTICK_SPEED_BAND;
/** Outer edge of run tier (speed ×⅔). Beyond this → full speed until physical rim. */
export const FOOTBALL_JOYSTICK_LEN_RUN_END = FOOTBALL_JOYSTICK_LEN_DEADZONE + 2 * FOOTBALL_JOYSTICK_SPEED_BAND;
...
export function packFootballAxis(dir, mag) {
...
export function unpackFootballAxis(u8) {
    const byte = u8 & 0xff;
    const dir = byte & 63;
    const mag = byte >> 6;
    return { dir, mag };
...
export function footballDirToAngle(dir) {
    const d = ((dir % FOOTBALL_DIR_STEPS) + FOOTBALL_DIR_STEPS) % FOOTBALL_DIR_STEPS;
    return (d / FOOTBALL_DIR_STEPS) * Math.PI * 2;
}
export function joystickToPackedFootballAxis(jx, jy) {
    const len = Math.hypot(jx, jy);
    if (len < FOOTBALL_JOYSTICK_LEN_DEADZONE)
        return packFootballAxis(0, 0);
    ...
    let mag = 3;
    if (len < FOOTBALL_JOYSTICK_LEN_WALK_END)
        mag = 1;
    else if (len < FOOTBALL_JOYSTICK_LEN_RUN_END)
        mag = 2;
    return packFootballAxis(step, mag);
}
export function packedAxisToVelocity(u8, maxSpeed) {
